...
Linear skew matches → Avellaneda-Stoikov inventory adjustment:
$$delta = gamma * q * sigma^2$$
Linear in inventory position ($q$), scaled by risk aversion ($gamma$).
...
$$Delta I = I_1 - I_0 quad "where" quad I = ubrace{sum_j p_j * abs{R_j - L_j}}_{"net coverage"}$$
...
$$C = {obrace{sum_j R_j * p_j}^{"total reserves"}}/{ubrace{sum_j L_j * p_j}_{"total liabilities"}}$$
...
2. **Coverage-weighted emissions** (see [Liquid Staking §3.2](/docs/2.4-liquid-staking#32-liquidity-provider-rewards)) and lower fees on coverage-improving trades (see [Spread & Fees §3.5](/docs/1.1.4-spread-fees#35-final-spread)) increase incentives for pools with low coverage
3. **Cooperative arbitrage** (see [Toxic Flow Mitigation §3.6](/docs/1.1.6-toxic-flow-mitigation#36-cooperative-arbitrage)) helps restore equilibrium faster when coverage slips via rebates that reward early rebalancing
