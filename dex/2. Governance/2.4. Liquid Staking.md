...
$$V_{btr} = sBTR_{balance} * btr_{price} * govMultiplier / 10000$$
...
$$Reward_{asset} = Total_sLP_{emissions} * weight_{pool} * weight_{asset}$$

where:
- $weight_{pool}$ ∝ $TVL_{pool} / Total_{TVL}$
- $weight_{asset}$ ∝ $(coverage * utilization)_{asset}$
...
where:
- $V_{btr}$ = voting value from sBTR = $sBTR_{balance} * BTR_{price} * govMultiplier / 10000$
- $V_{lp}$ = voting value from LP = $sum(sLP_{balance} * LP_{price} * lpBaseMultiplier / 10000)$ across all pools
- $V_{boost}$ = boosted voting value = $min(V_{lp}, boostCap * V_{btr} / 10000) * lpBoostedMultiplier / 10000$
