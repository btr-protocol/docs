...
description: "### BTR RISK DISCLAIMER"
audience: both
type: reference
status: live
phase: n/a
...
- **Staking**: Locking LP tokens (sLP) or governance tokens (sBTR) to receive protocol-distributed share and voting rights
...
The protocol relies on smart contracts that are **open-source** and available for public inspection on GitHub. Multiple internal audit cohorts have completed reviews of the BTR contract stack (see [Audit Reports](./audit-reports.md) for the index). Third-party audits remain in scope before mainnet General Availability. Users should treat all software as experimental until third-party audit reports are published. Undiscovered vulnerabilities could result in partial or total loss of deposited funds.
...
BTR employs multiple security measures including internal testing, circuit breakers, and monitoring, but no system can be 100% secure. Multiple internal audit cohorts have completed reviews of the BTR contract stack (see [Audit Reports](./audit-reports.md) for the index); third-party audits remain in scope before mainnet General Availability, and all software should be treated as experimental until those reports are published. You acknowledge these inherent risks.
...
8. **Audit Status**: You acknowledge that multiple internal audit cohorts have completed reviews of the BTR contract stack (see [Audit Reports](./audit-reports.md) for the index), that third-party audits remain in scope before mainnet General Availability, and that all software should be treated as experimental until third-party audit reports are published. Smart contracts are open-source and available for public review, but absence of third-party audit findings does not guarantee absence of vulnerabilities.
