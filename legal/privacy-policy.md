---
title: "BTR PRIVACY POLICY"
description: "BTR Supply and its contributors (collectively \"BTR Supply\", \"BTR\", “we”, “us”, “our”) respect the privacy of its customers, users and visitors of its services"
audience: both
type: reference
status: live
phase: n/a
...
- Transaction Information: When you commence using our Services, Ethereum blockchain and other public blockchains provide transparency into transactions. As this information is linked to your Wallet Address, it will almost never identify you as an individual but it will provide us a clear insight into your transactions which is required to perform the Services. This includes BTR ALM vault deposits, redemption requests (synchronous or ERC-7540 asynchronous), claim transactions, share transfers, operator approvals, governance interactions, intent openings via the Swapper singleton, and atomic-swap routing through pinned routers / settlers. All of this data is observable on-chain by anyone with access to the relevant blockchain RPC and is not under BTR's control.
- ALM Vault Holder Visibility: Your Wallet Address holding ALM vault shares, your pending redeem-ticket cohort, your claimable balance, your operator approvals (ERC-7540), and your principal share within multi-holder Dex adapters are all on-chain and queryable by third parties. BTR does not anonymize or obfuscate this data.
- Intent and Cross-Chain Routing: When you initiate an ERC-7683 intent or atomic swap through the Interface, your Wallet Address, the input / output token pair, the input amount, the deadline, and the route payload are visible to the chosen aggregator (LiFi, Unizen, Squid, Rango, Socket, others), to the pinned settler, and to any solver participating in fill discovery. BTR has no control over these third parties' privacy practices.
...
## 10. ALM-SPECIFIC DATA CONSIDERATIONS

### 10.1. On-Chain Data Is Public

BTR ALM operations execute entirely on public blockchains. The following information is therefore public, immutable, and not deletable by BTR or by you:
- Vault deposit and redemption transactions, including amounts, timestamps, and counterparty addresses
- Share balances, transfers, and operator approvals
- Adapter allocation actions (`pull` / `withdraw`) attributable to the vault
- Intent openings, settlements, refunds, and late-refund claims at the Swapper
- Governance proposal participation (forward-looking, post BTR-token issuance)

Your right to erasure (Section 5.1) does not extend to immutable on-chain data; it can only apply to data BTR controls off-chain.

### 10.2. Third-Party Aggregators and Settlers

Atomic-swap routes pass through aggregator services (LiFi and others). ERC-7683 intents are filled by external solvers via a pinned settler. Each of these third parties operates under its own privacy practices, which BTR does not audit and cannot guarantee. Where you initiate a swap or intent through the Interface, you should review the privacy policies of the relevant aggregator / settler.

### 10.3. RPC Endpoint Telemetry

The Interface dispatches read and write calls to public or commercial RPC endpoints (Alchemy, Infura, public chain RPCs, others). Those providers may log your IP address, the calls you make, and metadata about your session. BTR's choice of RPC provider is itself disclosed in the Interface configuration where applicable; you may use a self-hosted RPC to limit exposure.

### 10.4. Cross-Chain Considerations (Forward-Looking)

Forward-looking cross-chain ALM allocations will produce on-chain transaction history on each participating chain. Privacy across chains is correlated by your Wallet Address; using the same address across multiple chains exposes your full multi-chain activity to anyone running the appropriate analytics.
