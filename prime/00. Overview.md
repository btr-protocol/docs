...
flowchart LR
  ...
  C --> D[Feature matrix<br/>61 assembled / 32 active @ 1 alt - Phase 42Z]
...
| 03 | [Features](./03.%20Features.md) | 29 MTF core + 12 Renko-tracker + 8 cross-asset (per alt) + 12 microstructure = **61 features** at single-alt canonical config; multi-timeframe scheme. |
...
**Phase 42Z (live as of 2026-05-17):** 61→32 feature compression deployed per 4-expert panel sign-off; 8 hard gates ([§03 §3.7.2](./03.%20Features.md)) run in CI before each model promotion. 61-baseline retained for ablation via `--baseline-features`.
...
**Feature-economy note (Phases 42Y/42Z).** Phase 42Y's redundancy audit + 4-expert panel review (statistics, microstructure, GBM-ML, risk-adversarial) returned a unanimous APPROVE-WITH-MODS for a compressed shortlist of **32 features** (range 30-35; original audit said 28, raised via residualisation). **Phase 42Z (live 2026-05-17)** promoted the 32-feature set to production. The 8 hard gates (per-fold permutation importance LO/SO-split, tail-event subset CV, sensor-fault and stale-feed sims, et al.) are now **CI promotion gates** run before each model deploy, not research blockers. The 61-baseline is retained for ablation. See [§03 §3.7](./03.%20Features.md) and [§10 §6.6](./10.%20Open%20Questions.md).
