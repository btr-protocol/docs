...
flowchart LR
  ...
  C --> D[Feature matrix<br/>29 MTF + 12 renko + 8 cross/alt + 12 microstructure = 61 @ 1 alt]
...
**Feature-economy note (Phase 42Y).** A Phase 42Y redundancy audit + 4-expert panel review (statistics, microstructure, GBM-ML, risk-adversarial) returned a unanimous APPROVE-WITH-MODS for a compressed shortlist of **$\sim 32$ features** (range 30-35; original audit said 28, raised via residualisation). The compressed-set work is **research-gated** and **not yet in production**: eight hard pre-deploy gates (per-fold permutation importance LO/SO-split, tail-event subset CV, sensor-fault and stale-feed sims, et al.) plus the per-fold feature-selection refactor must close first. Production remains **61 features** until those gates clear. See [§03 §3.7](./03.%20Features.md) and [§10 §6.6](./10.%20Open%20Questions.md).
