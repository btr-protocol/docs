...
> **See also**: [AMM Landscape](/docs/AMM-Landscape) for side-by-side peer comparison · [Manifesto §5.2](/docs/Manifesto) for the multi-asset unified-pool rationale · [Foundations §10](/docs/Foundations) for OrbSwap's complementary sphere-invariant approach.
...
BTR's anchor-tree (see `1.1. Pricing`) means an oracle update on the root re-prices every leaf in the same block. UniV3 pools must each be arbed individually after an external price move — the arb is paid for by LPs (toxic-flow / MEV leak).
...
Empirically this is 30-60% LVR reduction on volatile pairs; we treat it as a multiplicative factor `β_oracle ∈ [1.3, 1.6]` on net-fee revenue.
...
`E_BTR / E_V3 ≈ w_hub · (N-1) · γ_contention + w_nonhub · μ_multihop + β_oracle`

where:

- `γ_contention ∈ [0.5, 1.0]` discount for concurrent-trade contention (typically 0.7-0.9)
