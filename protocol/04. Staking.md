...
description: "Governance token and LP token staking with voting power, rewards, and timelocked exit"
audience: both
type: reference
status: live
phase: n/a
...
**Liquid staking** in the BTR Protocol enables users to:
...
- **Stake Vault shares** (sVault, forward-looking) for emission share parallel to sLP - post-Phase-30 generic `IStakable` enables this without protocol redesign.
- **Manage cooldowns** on unstaking to prevent flash-loan voting attacks

> **Generic `IStakable` (post-Phase-30)**: Staking module no longer hard-codes BTR + LP-token recipients. Any contract implementing `IStakable` (cooldown, accrue, claim) can be whitelisted by governance as an emission target - paving the way for sVault and future stake pools without contract redeploys.
...
> **Key differentiator**: Unlike veCRV (non-transferable) or veAERO (NFT, non-fungible), sBTR is a standard ERC-20 that can be freely transferred. See [BTR Token §4.1](./08.%20BTR%20Token.md#41-liquid-staked-tokens) for comparison.
...
*\*Illustrative projection only. Not a forecast. Assumes static TVL, constant token price, and uninterrupted emission schedule. Actual APY varies with TVL, token price, governance changes, and emission halving. See [Risk Disclaimer](../legal/risk-disclaimer.md#governance-and-emissions-risk).*
...
> **Note**: This is the linear component. For complete formula including quadratic damping, see [Governance §2.3](./07.%20Governance.md#23-quadratic-damping).
...
- Bridgeable cross-chain via `Bridge` (see [Bridge](./06.%20Bridge.md))
...
**Coverage weighting** (see [Inventory Management §2](../dex/1.%20AIMM/1.1.%20Pricing/1.1.1.%20Inventory%20Management.md#2-coverage-ratio)):
...
### 3.4. Vault-Share Staking (sVault, forward-looking)

The generic `IStakable` interface (post-Phase-30) enables Vault-share staking as a first-class emission target - `sVault` receipts will accrue rewards via the same `Distributor` campaign pipeline as `sLP` / `sBTR`, with no contract redeploys required. See [Distributor](./05.%20Distributor.md). Boost matching (sBTR vs sVault) mirrors the sLP formula once the Vault stake pool ships.
...
> For complete formula including quadratic damping, see [Governance §2.3](./07.%20Governance.md#23-quadratic-damping).
...
- [Governance](./07.%20Governance.md) -Voting power calculation, proposal lifecycle
- [Treasury](./03.%20Treasury.md) -Emission minting and routing
- [Emissions](./09.%20Emissions.md) -Emission schedule details
