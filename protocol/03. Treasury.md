...
- **Protocol fee collection**: Receives ∀ protocol fees — DEX swap/flash/withdrawal-haircut + Vault mgmt/perf/exit — via `ac.treasury()` SSoT.
...
- **Distributor**: campaign-based emissions to ∀ registered stake pools — DEX sLP holders, sBTR stakers, and (forward-looking) Vault-share stakers.
...
- **Bridge**: LayerZero v2 OApp — bridges BTR token (and sBTR/sLP/sVault receipts where enabled) across chains for ∀ protocol surfaces (see `Bridge.sol`).
