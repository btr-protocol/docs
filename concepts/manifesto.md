---
title: "BTR Manifesto: ALM-First, AIMM-Next"
description: "Adaptive Inventory Market Maker -Bringing Order Book Intelligence to On-Chain Liquidity"
audience: both
type: explanation
status: live
phase: n/a
...
publish: true
---
# BTR Manifesto: ALM-First, AIMM-Next
...
After months of development on existing DEX stacks (primarily Uniswap V3/V4, Raydium CLMM, Meteora DLMM, and Orca Whirlpools), we identified a structural constraint that informs BTR's design.
...
This is why BTR ships **two complementary products**. **BTR Supply** brings ALM adapters to existing CL DEXs (Uniswap V3/V4, PancakeSwap V3/Infinity, Aerodrome Slipstream, and peers) because that is where users and liquidity live today, we make the existing reality work better. In parallel, **BTR DEX (AIMM)** delivers the architecture we believe is the better long-term path: multi-asset, spline-based, inventory-aware, natively single-sided. **BTR Supply ships Phase 1; BTR DEX (AIMM) ships Phase 2 as one venue among many in Supply's adapter set.**
...
### 1.3. BTR Supply: The Launch Product
...
**Performance accounting**: `harvestPerformance()` is the **only** function that crystallizes performance fees, against the vault-level `(hwmAssets, hwmSupply)` HWM pair. `_accrue` never touches HWM. Performance fee charged on strictly positive `gain = ta − hwmAssets × sup / hS`.

**`depositCap`**: governance-set per-vault deposit ceiling. Throttles growth during early phases, hardens kill-switch semantics, and aligns vault size with adapter liquidity depth.
...
| Internal oracle from pool state | LFJ Liquidity Book, Bancor, Curve V2 | TWAP / active-state as price reference | Dual-window EMA (5min/1hr) instead of active-bin or single accumulator |
...
The pool behaves less like a passive option writer and more like an informed counterparty that charges appropriately for risk.
...
- DAO could run its own bot with 100% donation rate → maximal reputation → **aims to close** LVR loop
...
V4 hooks demonstrate strong demand for adaptive AMM features; AIMM explores a complementary clean-slate design while BTR Supply ships those features to V4 LPs today.
...
### 12.1. Why AIMM Is a Separate Protocol, Not a V5
...
These are fundamentally different designs. AIMM and Uniswap target different design points; they coexist, not substitute.
...
AIMM explores the post-strict-invariant design space; BTR Supply pragmatically improves LP outcomes on today's CL venues.

**Two products, sequenced rollout.** BTR Supply ships Phase 1 (ALM on UniV3, UniV4, PancakeV3, PancakeInfinity, Algebra, Ramses); BTR DEX (AIMM) ships Phase 2 and becomes an additional Supply adapter target. BTR Supply optimizes liquidity management on the concentrated-liquidity venues where users and volume live today-CL liquidity management is more capital-intensive than we would like, but it is the reality of on-chain liquidity in 2026, and someone has to do it well. BTR DEX (AIMM) delivers the architecture we believe is the better long-term path: multi-asset, spline-based, inventory-aware, natively single-sided. The two products fund and inform each other.
